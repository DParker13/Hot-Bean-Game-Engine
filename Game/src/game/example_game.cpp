...
ExampleGame::ExampleGame(const std::string& config_path, std::shared_ptr<IComponentFactory> component_factory)
...
void ExampleGame::OnStart() {
    ...
    std::shared_ptr<ISerializer> serializer = std::make_shared<HBE::Default::DefaultSceneSerializer>(g_app.GetComponentFactory());
