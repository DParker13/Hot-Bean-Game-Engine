...
namespace HBE::Core {
    ...
    struct Octree2DNode {
        ...
        void Insert(T *item, const glm::vec2 position, const int depth,
                    std::unordered_map<T, Octree2DNode<T> *> item_to_node) {
            if (depth > 0) {
         ...
         */
        bool IsLeaf() const {
            return m_child_nodes[0] == nullptr && m_child_nodes[1] == nullptr && m_child_nodes[2] == nullptr &&
                   m_child_nodes[3] == nullptr;
        }
        ...
        bool WithinBounds(glm::vec2 position) const {
            return position.x > m_bounds.x && position.x <= m_bounds.x + m_bounds.z && position.y > m_bounds.y &&
        ...
        void CreateChildNodes() {
            ...
            m_child_nodes[3] =
                new Octree2DNode(glm::ivec3(m_bounds.x + child_size, m_bounds.y + child_size, child_size));
        }
