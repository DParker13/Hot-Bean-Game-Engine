...
namespace Core::Managers {
    ...
    class ECSManager {
        public:
            ...
            ComponentManager& GetComponentManager() { return *m_component_manager; }
            ...
            void RemoveComponent(Entity entity) {
                try {
                    ComponentType component_type = m_component_manager->GetComponentType<T>();
                    m_component_manager->RemoveComponent<T>(entity);
                    Signature signature = m_entity_manager->SetSignature(entity, component_type, false);
                    m_system_manager->EntitySignatureChanged(entity, signature);
                }
                catch(const std::exception& e) {
                    return;
            ...
            void SetSignature(IArchetype* archetype) {
                try {
                    if (!m_system_manager->IsSystemRegistered<S>()) {
                    ...
                    for (std::string component_name : component_names) {
                        if (m_component_manager->IsComponentRegistered(component_name)) {
                            signature.set(m_component_manager->GetComponentType(component_name));
                        }
                        else {
            ...
            void SetSignature() {
                try {
                    ...
                    Signature& signature = m_system_manager->GetSignature<S>();
                    ((signature.set(m_component_manager->IsComponentRegistered<Cs>() ?
                            m_component_manager->GetComponentType<Cs>() :
                            m_component_manager->RegisterComponentType<Cs>())), ...);
