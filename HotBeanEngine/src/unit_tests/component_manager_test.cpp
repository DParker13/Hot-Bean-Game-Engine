...
TEST_CASE("ComponentManager: Component Removal") {
    ...
    SECTION("Remove non-registered component throws") {
        REQUIRE_THROWS_AS(component_manager.RemoveComponent<TestComponent>(entity_1), ComponentNotRegisteredException);
    }
...
TEST_CASE("ComponentManager: Component Type Queries") {
    ...
    SECTION("Get component ID throws if not registered") {
        REQUIRE_THROWS_AS(component_manager.GetComponentID<TestComponent>(), ComponentNotRegisteredException);
    }
}
