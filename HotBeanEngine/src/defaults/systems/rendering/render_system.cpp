...
namespace HBE::Default::Systems {
    ...
    /**
     * @brief Called when an entity is added to the system. Ensures a layer texture exists for the entity's layer.
     * @param entity The entity ID being added.
     */
    void RenderSystem::OnEntityAdded(EntityID entity) { CreateLayerTextureForEntity(entity); }
    ...
    /**
     * @brief Called every frame to update system state before rendering. Clears the set of used layers.
     */
     ...
     *
     * Iterates over all entities and all active cameras, rendering each entity to the appropriate layer texture.
    ...
    void RenderSystem::RenderTextureToLayer(EntityID camera_entity, EntityID entity) {
        ...
        SDL_RenderTextureRotated(renderer, texture.m_texture, NULL, &rect, transform.m_world_rotation, NULL,
                                 SDL_FLIP_NONE);
     ...
     *
     * Creates a temporary texture, draws a red rectangle, and overlays it on the entity's layer.
     */
    void RenderSystem::DrawDebugRect(const Texture &texture, const Transform2D &transform, const SDL_FRect *rect) {
        ...
        SDL_Texture *debug_texture =
            SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
                              static_cast<int>(texture.m_size.x), static_cast<int>(texture.m_size.y));
        ...
        SDL_SetRenderTarget(renderer, m_layers[transform.m_layer]);
        SDL_RenderTextureRotated(renderer, debug_texture, NULL, rect, transform.m_world_rotation, NULL, SDL_FLIP_NONE);
