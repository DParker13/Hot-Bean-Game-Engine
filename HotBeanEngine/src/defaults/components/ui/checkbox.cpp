...
namespace HBE::Default::Components {
    ...
    void Checkbox::Deserialize(YAML::Node &node) {
        if (node["text"]) {
    ...
    void Checkbox::Serialize(YAML::Emitter &out) const {
        if (!m_text.empty()) {
     ...
     */
    std::string Checkbox::GetString() const { return m_text; }
     ...
     */
    const char *Checkbox::GetChar() const { return m_text.data(); }

    void Checkbox::RenderProperties(int &id) {
