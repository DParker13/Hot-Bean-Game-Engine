...
namespace Systems {
    CollisionSystem::CollisionSystem(App& app, int resolution)
        : System(app) {
        app.GetCoreManager().RegisterSystem<CollisionSystem>(this);
        
        app.GetCoreManager().SetSignature<CollisionSystem, Transform2D, RigidBody, Collider2D>();
    ...
    bool CollisionSystem::CheckCollision(Entity entity_one, Entity entity_two) {
        ...
        auto& transform_one = coreManager.GetComponent<Transform2D>(entity_one);
        auto& transform_two = coreManager.GetComponent<Transform2D>(entity_two);

        auto& collider_one = coreManager.GetComponent<Collider2D>(entity_one);
        auto& collider_two = coreManager.GetComponent<Collider2D>(entity_two);
    ...
    void CollisionSystem::ResolveCollision(Entity entity_one, Entity entity_two) {
        ...
        if (collider_one._is_trigger || collider_two._is_trigger) {
            ...
            auto& transform_one = coreManager.GetComponent<Transform2D>(entity_one);
            auto& transform_two = coreManager.GetComponent<Transform2D>(entity_two);
            //auto& rigidbody_one = _coreManager.GetComponent<RigidBody>(entity_one);
            //auto& rigidbody_two = _coreManager.GetComponent<RigidBody>(entity_two);
